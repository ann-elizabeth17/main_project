...
class DessertsPage extends StatelessWidget {
  ...
  Future<void> _deleteFood(String id) async {
    await FirebaseFirestore.instance.collection('desserts').doc(id).delete();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Desserts Foods'),
        backgroundColor: const Color.fromARGB(255, 43, 11, 11),
        foregroundColor: Colors.white,
      ),
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('desserts').snapshots(),
...
class _AddEditFoodPageState extends State<AddEditFoodPage> {
  ...
  Future<void> _saveFoodItem() async {
    ...
    if (widget.isEdit) {
      ...
      final docRef = await FirebaseFirestore.instance.collection('desserts').add(foodData);
