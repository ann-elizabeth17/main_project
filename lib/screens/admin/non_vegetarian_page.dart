...
class NonVegetarianPage extends StatelessWidget {
  const NonVegetarianPage({Key? key}) : super(key: key);

  Future<void> _deleteFood(String id) async {
    await FirebaseFirestore.instance.collection('nonVegetarian').doc(id).delete();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Non-Vegetarian Foods'),
        backgroundColor: const Color.fromARGB(255, 43, 11, 11),
        foregroundColor: Colors.white,
      ),
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('nonVegetarian').snapshots(),
        builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
          ...
          if (snapshot.data!.docs.isEmpty) {
            return const Center(
              child: Text(
                'No items in Non-Vegetarian Foods',
  ...
  Future<void> _saveFoodItem() async {
    ...
    if (widget.isEdit) {
      await FirebaseFirestore.instance
          .collection('nonVegetarian')
          .doc(widget.foodId)
          .update({...foodData, 'id': widget.foodId});
    } else {
      final docRef = await FirebaseFirestore.instance.collection('nonVegetarian').add(foodData);
