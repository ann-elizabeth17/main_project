...
class RicePage extends StatelessWidget {
  ...
  Future<void> _deleteFood(String id) async {
    await FirebaseFirestore.instance.collection('rice').doc(id).delete();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Rice Foods'),
        backgroundColor: const Color.fromARGB(255, 43, 11, 11),
        foregroundColor: Colors.white,
      ),
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('rice').snapshots(),
...
class _AddEditFoodPageState extends State<AddEditFoodPage> {
  ...
  Future<void> _saveFoodItem() async {
    ...
    if (widget.isEdit) {
      ...
      final docRef = await FirebaseFirestore.instance.collection('rice').add(foodData);
