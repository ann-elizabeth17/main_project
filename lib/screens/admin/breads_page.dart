...
class BreadsPage extends StatelessWidget {
  const BreadsPage({Key? key}) : super(key: key);

  Future<void> _deleteFood(String id) async {
    await FirebaseFirestore.instance.collection('breads').doc(id).delete();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Breads Foods'),
        backgroundColor: const Color.fromARGB(255, 43, 11, 11),
        foregroundColor: Colors.white,
      ),
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('breads').snapshots(),
...
class _AddEditFoodPageState extends State<AddEditFoodPage> {
  ...
  Future<void> _saveFoodItem() async {
    ...
    if (widget.isEdit) {
      ...
      final docRef = await FirebaseFirestore.instance.collection('breads').add(foodData);
