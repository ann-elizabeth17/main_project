...
class StreetFoodsPage extends StatelessWidget {
  const StreetFoodsPage({Key? key}) : super(key: key);

  Future<void> _deleteFood(String id) async {
    await FirebaseFirestore.instance.collection('streetFoods').doc(id).delete();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Street Foods'),
        backgroundColor: const Color.fromARGB(255, 43, 11, 11),
        foregroundColor: Colors.white,
      ),
      body: StreamBuilder(
        stream: FirebaseFirestore.instance.collection('streetFoods').snapshots(),
...
class _AddEditFoodPageState extends State<AddEditFoodPage> {
  ...
  void initState() {
    ...
    if (widget.isEdit) {
      ...
      _ingredientsController = TextEditingController(text: widget.foodItem!['ingredients']);
      _nutritionController = TextEditingController(text: widget.foodItem!['nutrition']);
      _deliveryDetailsController = TextEditingController(text: widget.foodItem!['deliveryDetails']);
      _ratingController = TextEditingController(text: widget.foodItem!['rating']);
  ...
  Future<void> _saveFoodItem() async {
    ...
    if (widget.isEdit) {
      await FirebaseFirestore.instance
          .collection('streetFoods')
          .doc(widget.foodId)
          .update({...foodData, 'id': widget.foodId});
    } else {
      final docRef = await FirebaseFirestore.instance.collection('streetFoods').add(foodData);
